...
#cat ../lexicon-lengthLabel.txt | awk '{for (i=2;i<=NF;i++) print $i}' | sort -u > nonsilence_phones.txt    #--2 phone units for 1 vowel (i.e.: vowel /a/ is represented [a] and [aL])
...
#cat ../lexicon-lengthLabel.txt | sed 's/(.)//' > lexicon.txt   #-- vowels are length contrasted in the acoustic dictionary 
